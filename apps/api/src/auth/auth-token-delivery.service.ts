...
export class AuthTokenDeliveryService {
  ...
  async sendEmailVerification(email: string, token: string): Promise<void> {
    await this.deliver('email verification', email, `${this.env.WEB_ORIGIN}/verify-email?token=${encodeURIComponent(token)}`);
  }

  async sendPasswordReset(email: string, token: string): Promise<void> {
    await this.deliver('password reset', email, `${this.env.WEB_ORIGIN}/reset-password?token=${encodeURIComponent(token)}`);
  }

  private async deliver(kind: string, email: string, url: string): Promise<void> {
