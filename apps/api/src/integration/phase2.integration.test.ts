...
void test('Phase 2 Career Passport versions candidate-owned professional state', async (t) => {
  ...
  try {
    ...
    await t.test('initialization creates private candidate identity and first approved version', async () => {
    ...
    await t.test('overview update creates a new version and supersedes the previous snapshot', async () => {
    ...
    await t.test('section replacement preserves unrelated profile data in the next snapshot', async () => {
    ...
    await t.test('experience and education updates remain candidate-owned versioned state', async () => {
    ...
    await t.test('privacy settings remain separate from professional profile versions', async () => {
    ...
    if (userId) {
      await database.outboxEvent.deleteMany({ where: { aggregateType: 'User', aggregateId: userId } });
