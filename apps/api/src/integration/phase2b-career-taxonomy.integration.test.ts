...
void test('Career Passport core contact awards and taxonomy stay immutable and lossless', async (t) => {
  ...
  try {
    ...
    await t.test(
      'professional contact information versions separately from account identity',
      async () => {
    ...
    await t.test(
      'awards preserve submitted order and untouched sections across snapshots',
      async () => {
    ...
    await t.test(
      'recognized and unknown extension metadata survives unrelated version changes',
      async () => {
        ...
        assert.equal(
          next.currentProfileVersion!.versionNumber,
          withSections.currentProfileVersion!.versionNumber + 1,
        );
        assert.deepEqual(
