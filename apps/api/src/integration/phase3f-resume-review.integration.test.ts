...
if (!connectionString) throw new Error('DATABASE_URL is required for Phase 3F review tests.');
...
void test('Phase 3F resume review read model stays candidate-owned and privacy bounded', async (t) => {
  ...
  try {
    ...
    await t.test('review response omits source object keys and raw extraction text', async () => {
      ...
      assert.equal(serialized.includes('RAW_PRIVATE_EXTRACTION_TEXT_MUST_NOT_LEAVE_REVIEW_API'), false);
