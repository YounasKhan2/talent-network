...
void test('dispatches OCR-required events with stable OCR job identity', async () => {
  ...
  const published = await dispatchResumeOcrEvents(fixture.database, fixture.queue);

  assert.equal(published, 1);
  assert.deepEqual(fixture.addedJobs[0], {
    name: RESUME_OCR_QUEUE,
    data: {
      resumeVersionId: 'resume-version-1',
      processingPipelineVersion: 'resume-v1',
    ...
    options: {
      jobId: resumeOcrJobId('resume-version-1', 'resume-v1', 'extraction-1'),
      attempts: 3,
...
void test('rejects malformed or mismatched OCR events without publishing', async () => {
  const fixture = createFixture([
    {
      id: 'event-bad',
...
void test('OCR queue failure keeps outbox event unpublished for retry', async () => {
  const fixture = createFixture(
    [
      {
        id: 'event-fail',
...
function createFixture(seed: MutableEvent[], failQueue = false) {
  ...
  const queue = {
    add: (name: string, data: ResumeOcrJobData, options: Record<string, unknown>) => {
  ...
  } as unknown as Queue<ResumeOcrJobData>;
